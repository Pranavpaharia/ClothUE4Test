...
void FindMeshInfo(const aiScene* scene, aiNode* node, FReturnedData& result)
{
	...
	for (uint32 i = 0; i < node->mNumMeshes; i++)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("FindMeshInfo1: %d %d"), meshidx, i);
		aiMesh *mesh = scene->mMeshes[i];
		aiMaterial *mat = scene->mMaterials[mesh->mMaterialIndex];
		aiString texturePath = aiString("samp");
		mat->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath);
		result.meshInfo[i].TexturePath = FString(texturePath.C_Str());
		FMeshInfo &mi = result.meshInfo[i];
