		...
		if (true) {
			...
			for (int j = 0; j < childrenBones.Num(); j++)
			{
				...
				if (FVector::DotProduct(vec2, vec3) > 0) {
					FName childBoneName1 = _refCloth->GetBoneName(childrenBones[maxChildIndex]);
					FVector childBonePosition2 = _refCloth->GetOwner()->GetTransform().TransformPosition(_refCloth->GetBoneLocation(childBoneName1, EBoneSpaces::ComponentSpace));
					//FVector vec1 = _vertices[i].Position - maxBonePosition;
					FVector vec4 = vert - maxBonePosition;
					FVector vec5 = childBonePosition2 - maxBonePosition;
					//DrawDebugPoint(_refCloth->GetOwner()->GetWorld(), childBonePosition, 10, FColor::Cyan, true, 1, 0);
